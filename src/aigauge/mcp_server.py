...
def _account_rows(config: Config, cache: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for account_id in account_ids:
        snapshot = cached.get(account_id, {})
        ...
        for metric in metrics:
            ...
            if percent is not None:
                eligible.append((percent, metric))
        max_percent = max((item[0] for item in eligible), default=None)
...
def _check_usage_guard(
    ...
    if cache.get("schema_version") != CACHE_SCHEMA_VERSION:
        return _blocked(
            ...
            "AI Gauge usage cache is missing or incompatible.",
            account_id=account_id,
            guard_configured=True,
        )
    row = next(
        (item for item in _account_rows(config, cache) if item["account_id"] == account_id),
        None,
    )
    if row is None:
        return _blocked(
